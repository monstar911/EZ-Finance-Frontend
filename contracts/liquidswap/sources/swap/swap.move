...
module liquid::swap {
    ...
    friend liquid::router;

    const ZERO_ACCOUNT: address = @zero_liquid;
    const DEFAULT_ADMIN: address = @default_admin_liquid;
    const RESOURCE_ACCOUNT: address = @liquid;
    const DEV: address = @dev_liquid;
    ...
    */
    ...
    public(friend) fun create_pair<X, Y>(
        ...
        let lp_name: string::String = string::utf8(b"Liquid-");
        ...
        if (string::length(&lp_name) > MAX_COIN_NAME_LENGTH) {
            lp_name = string::utf8(b"Liquid LPs");
        };
