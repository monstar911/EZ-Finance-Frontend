module auxexchange::router {
    use auxexchange::swap;
    use std::signer;
    ...
    public fun swap_exact_x_to_y_direct_external<X, Y>(x_in: coin::Coin<X>): coin::Coin<Y> {
        ...
        add_swap_event_with_address_internal<X, Y>(@zero_aux, x_in_amount, 0, 0, y_out_amount);
        y_out
    }
    ...
    public fun swap_x_to_exact_y_direct_external<X, Y>(x_in: coin::Coin<X>, y_out_amount:u64): (coin::Coin<X>, coin::Coin<Y>) {
        ...
        add_swap_event_with_address_internal<X, Y>(@zero_aux, x_in_withdraw_amount, 0, 0, y_out_amount);
        (x_in_left, y_out)
    }
